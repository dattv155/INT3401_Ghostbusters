...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # if ghost jail then ghost is here, p[position] = 1, p[other] = 0
        ...
        # for each position (of ghost)
        # p[is ghost] = p[true distance ^ noisy distance] = p[true distance] * p[noisy distance | true distance]
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
            ...
            # generate new pos and newpos's prob
            for newPos, prob in newPosDist.items():
                # calc p[noise distance ^ new pos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        # insert legal position to particles, beak when num(particles) = self.numParticles
        self.particles = []
    ...
    def observe(self, observation, gameState):
        ...
        # if ghost jail, set particle to self.getJailPostion
        if noisyDistance is None:
            self.particles = [self.getJailPosition() for i in range(self.numParticles)]
            return

        # calc prob for each pos
        allPossible = util.Counter()
